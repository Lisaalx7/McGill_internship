...
class BackEnvV0(BaseV0):
    ...
    def get_reward_dict(self, obs_dict):
        L5_error  = [[self.target_jnt_value[:3] - self.sim.data.qpos[:3].copy()]]
        pose_dist = np.linalg.norm(L5_error, axis=-1) #np.linalg.norm(obs_dict['pose_err'], axis=-1)
    ...
    def update_target(self, restore_sim=False):
        ...
        self.sim.data.qpos[:18] = self.target_jnt_value.copy()
        self.sim.forward()
